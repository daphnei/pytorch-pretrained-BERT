    ...
    def beam_search(self, initial_input, initial_state=None):
        ...
        for b in range(batch_size):
            ...
            for k in range(self.beam_size):
                new_sequence = torch.cat([initial_input[b],  words[b][k].expand(1)], 0)
                seq = Sequence(
                    output=new_sequence,
                    state=new_state[b],
        ...
        # Run beam search.
        for iteration in range(self.max_sequence_length - 1):
            ...
            input_feed = [c.output.reshape([1, -1]) for c in flattened_partial]
            ...
            for b in range(batch_size):
                ...
                for partial in partial_sequences_list[b]:
                    ...
                    while num_hyp < self.beam_size:
                        w = words[idx][k]
                        output = torch.cat([partial.output, w.unsqueeze(0)], 0)
        ...
        # sequences.
        for b in range(batch_size):
            if complete_sequences[b].size() == 0:
